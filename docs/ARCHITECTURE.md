...
├── ai.*            — RAG-анализ документации репозитория
├── c4isr.*         — Интегрированное управление и Live Comms
├── hardware.*      — Управление внешними физическими модулями
...
**Процедуры инлайн-анализа** — Несколько процедур tRPC в `server/routers.ts` используют LLM для задач по запросу: суммаризация статей, генерация оценки угроз и синтез данных из разных источников.

**Ask AI (RAG-анализ)** — Специализированный роутер `server/routers/ai.ts` реализует поиск по локальной документации репозитория. При запросе сервер считывает Markdown-файлы (`README.md`, `ARCHITECTURE.md` и др.), формирует контекстное окно для LLM и возвращает ответ, основанный на технических спецификациях проекта. Это позволяет аналитикам получать справку по работе платформы в реальном времени.
...
## Экспорт в SIEM (Splunk/QRadar)

Redroom поддерживает интеграцию с внешними системами управления событиями безопасности (SIEM). Утилита `server/siem.ts` позволяет форвардить критические события в формате JSON на настроенный вебхук.

Ключевые точки логирования:
- Аутентификация (регистрация, успешные/неудачные входы).
- Доступ к CMS и административные действия.
- Изменение конфигурации системы.
- Подключение/отключение внешних аппаратных модулей.

Конфигурация осуществляется через переменные `SIEM_WEBHOOK_URL` и `SIEM_PROVIDER` в файле `.env`.
